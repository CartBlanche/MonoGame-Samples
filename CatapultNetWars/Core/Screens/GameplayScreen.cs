...
namespace CatapultGame
{
	class GameplayScreen : GameScreen
	{
		...
		// internal NetworkSession NetworkSession { get; private set; } // Not available in MonoGame 3.8
		// PacketReader packetReader = new PacketReader (); // Not available in MonoGame 3.8
		// PacketWriter packetWriter = new PacketWriter (); // Not available in MonoGame 3.8
		...
		public void LoadAssets ()
		{
			...
			// NetworkSession = ScreenManager.Game.Services.GetService (typeof(NetworkSession)) as NetworkSession; // Not available in MonoGame 3.8
			// IsNetworking = NetworkSession != null; // Not available in MonoGame 3.8
			IsNetworking = false; // Disable networking for MonoGame 3.8
			...
			if (IsNetworking) {
				// Networking code disabled in MonoGame 3.8
				// Set up default single player game instead
	...
	public override void Update (GameTime gameTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
		{
			...
			if ((playerOne.Catapult.GameOver || playerTwo.Catapult.GameOver) &&
				...
				gameOver = true;
			if (IsNetworking) {
				// if (NetworkSession.IsHost) { // Not available in MonoGame 3.8
				...
				//	}
				// }
				// Simple fallback - play win sound for player 1 if they have higher score
				if (playerOne.Score > playerTwo.Score) {
					AudioManager.PlaySound ("gameOver_Win");
				} else {
					AudioManager.PlaySound ("gameOver_Lose");
			...
			// UpdateNetworkSession (); // Disabled - networking not available in MonoGame 3.8
		...
		// if (NetworkSession != null) { // Not available in MonoGame 3.8
		...
		*/
		...
		{
			if (gameOver) {
				Texture2D texture;			if (IsNetworking) {
				// if (NetworkSession.IsHost) { // Not available in MonoGame 3.8
				//	if (playerOne.Score > playerTwo.Score) {
			...
			if (IsNetworking) {
				if (isFirstPlayerTurn) {
					...
					// }
					// Simple fallback - assume player 1 is local
					text = !isDragging ? "Drag Anywhere to Fire" : "Release to Fire!";
				} else {
					// if (NetworkSession.IsHost) { // Not available in MonoGame 3.8
					//	// Prepare player one prompt message
					...
					//	text = !isDragging ? "Drag Anywhere to Fire" : "Release to Fire!";
					// }
					// Simple fallback - assume player 2 is local
					text = !isDragging ? "Drag Anywhere to Fire" : "Release to Fire!";
				}
			}
				else {
